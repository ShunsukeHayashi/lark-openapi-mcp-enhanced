...
export const genesisCreateBase: McpTool = {
  ...
  customHandler: async (client: lark.Client, params: any) => {
    ...
    const clientError = validateClient(client, 'create Lark Base');
    ...
    try {
      ...
      return createErrorResponse(error, {
        operation: 'create Lark Base',
        toolName: 'genesis.builtin.create_base',
      });
    }
  },
};
...
export const genesisAnalyzeRequirements: McpTool = {
  ...
  customHandler: async (client: lark.Client, params: any) => {
    // Validate client initialization
    const clientError = validateClient(client, 'analyze requirements');
    if (clientError) return clientError;
    ...
    try {
      ...
      return createErrorResponse(error, {
        ...
        toolName: 'genesis.builtin.analyze_requirements',
      });
    }
  },
};
...
export const genesisListTemplates: McpTool = {
  ...
  customHandler: async (client: lark.Client, params: any) => {
    try {
      ...
      const templates = category === 'all' ? getAllTemplates() : getTemplatesByCategory(category);
...
export const genesisOptimizeBase: McpTool = {
  ...
  schema: {
    data: z.object({
      ...
      optimizationGoals: z
        .array(z.enum(['performance', 'usability', 'automation', 'analytics']))
